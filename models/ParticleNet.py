...
class ParticleNetTaggerMultiGroups(nn.Module):
    ...
    def __init__(self,
                 ...
                 for_inference=False,
                 **kwargs):
        super(ParticleNetTaggerMultiGroups, self).__init__(**kwargs)
        self.num_groups = len(input_dims)
        self.convs = nn.ModuleList()
        for dim in input_dims:
            self.convs.append(FeatureConv(dim, embed_dim))
        self.pn = ParticleNet(input_dims=embed_dim,
                              num_classes=num_classes,
                              conv_params=conv_params,
                              fc_params=fc_params,
                              use_fusion=use_fusion,
    ...
    def forward(self, *args):
        ...
        assert len(args) == 3 * self.num_groups

        points, features, mask = [], [], []
        for i in range(self.num_groups):
            points_, features_, mask_ = args[i*3], args[i*3+1], args[i*3+2]
            points.append(points_)
            features.append(self.convs[i](features_ * mask_) * mask_)
            mask.append(mask_)
