...
class ParticleTransformerTaggerMultiGroups(nn.Module):
    ...
    def forward(self, *args):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            if self.share_embed == False:
                x = [self.input_embeds[i](x[i]) for i in range(self.num_groups)] # after embed: (seq_len, batch, embed_dim)
            else:
                x = [self.input_embed(x[i]) for i in range(self.num_groups)]
            x = torch.cat(x, dim=0)
